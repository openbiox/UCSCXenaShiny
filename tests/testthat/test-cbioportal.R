test_that("cBioPortal functions work with cbioportalR", {
  ...
  # Test that connection initialization works
  skip_on_cran()  # Skip network tests on CRAN
  studies_result <- tryCatch(
    get_cbioportal_studies(base_url = "public"),
    error = function(e) data.frame()
  )
  expect_true(is.data.frame(studies_result))
  # Should return studies if connection works
  if (nrow(studies_result) > 0) {
    expect_true("studyId" %in% names(studies_result))
    expect_true("name" %in% names(studies_result))
  }
