...
#' Get Feature Names from a UCSC Xena Dataset
#'
#' Fetches the list of available features (genes, probes, etc.) from a dense matrix dataset.
#'
#' @param dataset a UCSC Xena dataset name
#' @param host optional UCSC Xena host URL, auto-detected if NULL
#' @param n_max maximum number of features to return (default: 100)
#'
#' @return a character vector of feature names, or NULL if failed
#' @export
#'
#' @examples
#' \dontrun{
#' # Get features from a gene expression dataset
#' features <- get_dataset_features("TCGA.BRCA.sampleMap/HiSeqV2")
#' head(features)
#' }
get_dataset_features <- function(dataset, host = NULL, n_max = 100) {
  tryCatch({
    ...
    if (is.null(host)) {
      ...
      if (length(host) == 0) {
        warning("Dataset not found in XenaData")
        return(NULL)
      }
      host <- host[1]
    }
    
    # Use httr to call Xena API to get field list
    url <- paste0(host, "/data/features")
    response <- httr::POST(
      url,
      body = list(dataset = dataset, limit = n_max),
      encode = "json",
      httr::timeout(30)
    )
    ...
    if (httr::status_code(response) == 200) {
      ...
      features <- jsonlite::fromJSON(content_text)
      
      if (length(features) > 0) {
        message("Retrieved ", length(features), " features from dataset: ", dataset)
        return(features)
      }
    }
    
    warning("Failed to retrieve features from dataset: ", dataset)
    return(NULL)
    
  }, error = function(e) {
    warning("Error retrieving features: ", e$message)
    return(NULL)
  })
}
