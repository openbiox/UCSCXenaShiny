...
vis_gene_cross_omics = function(gene="TP53", 
  ...
  gene_mrna_md = gene_mrna %>% 
    ...
    dplyr::summarise(Exp = median(.data$Exp)) %>% as.data.frame() %>% 
  ...
  gene_mrna_comp = ggpubr::compare_means(Exp ~ type2, data = gene_mrna,
  ...
  label_name[1:12] = c("","Normal Exp","Tumor Exp","T vs. N(Wilcox)", 
  ...
  column_info = data.frame(
    ...
    name = label_name, 
    # group name
    group = c("TCGA", rep("Expression Profile", 3), rep("Mutation Profile", 3), rep("CNV Profile", 5), 
  ...
  position_arguments = funkyheatmap::position_arguments(expand_xmax  = 6, 
                                          col_annot_angle = 35, 
                                          col_width = 1.1,
                                          col_space = 0.2,
                                          col_annot_offset = 4)
...
vis_pathway_cross_omics = function(pw="HALLMARK_ADIPOGENESIS", 
  ...
  pw_mrna_comp = ggpubr::compare_means(Exp ~ type2, data = pw_mrna,
                               group.by = "tissue", method = "wilcox.test") 
  ...
  column_info = data.frame(
    ...
    # group name
    group = c("TCGA", rep("Expression Profile", 3), rep("Mutation Profile", 5), 
              rep("CNV Profile", 5)),
    # geom type
    geom = c("text", "bar","bar","text", "pie","bar","text", "bar","text", 'pie',
             "bar","text","bar","text"),
    # palette type
    palette = c(NA,"Tumor-Exp","Tumor-Exp",NA, "Mut Status", "Mut-Ratio", NA, "Mut-Count", NA, "CNV Status",
                "CNV-Amp",NA,"CNV-Del",NA), #"CNV-Pie"
    # column width
...
calc_pw_mut_cnv = function(tumor_samples = NULL){
  ...
  pw_mut_stat = tcga_PW_cross_raw$pw_mut_one[tcga_clinical_fine2$Sample,] %>% na.omit() %>% 
    tibble::rownames_to_column("Sample") %>%
  ...
  pw_cnv_posi_stat = tcga_PW_cross_raw$pw_cnv_posi %>% as.data.frame() %>%
    tibble::rownames_to_column("Sample") %>% 
    dplyr::inner_join(tcga_clinical_fine2[,1:2],.) %>%
    dplyr::group_by(.data$Cancer) %>%
    dplyr::summarise(across(!.data$Sample, mean)) %>% 
    as.data.frame()
  pw_cnv_nega_stat = tcga_PW_cross_raw$pw_cnv_nega %>% as.data.frame() %>%
    tibble::rownames_to_column("Sample") %>% 
    dplyr::inner_join(tcga_clinical_fine2[,1:2],.) %>%
    dplyr::group_by(.data$Cancer) %>%
    dplyr::summarise(across(!.data$Sample, mean)) %>% 
    as.data.frame()
  pw_cnv_zero_stat = tcga_PW_cross_raw$pw_cnv_zero %>% as.data.frame() %>%
    tibble::rownames_to_column("Sample") %>% 
    dplyr::inner_join(tcga_clinical_fine2[,1:2],.) %>%
    dplyr::group_by(.data$Cancer) %>%
    dplyr::summarise(across(!.data$Sample, mean)) %>% 
