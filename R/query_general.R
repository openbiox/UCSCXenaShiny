query_general_id <- function() {
  ...
  tcga_value_option <- list(
    "Tumor index" = tcga_index_value, "Immune Infiltration" = tcga_immune_value,
    "Pathway activity" = tcga_pathway_value, "Phenotype data" = tcga_phenotype_value
  )
  pcawg_value_option <- list(
    "Tumor index" = pcawg_index_value, "Immune Infiltration" = pcawg_immune_value,
  ...
  tcga_index_id <- list(
    ...
    `Microsatellite Instability` = list(
      all = sort(setdiff(
        colnames(tcga_index_value$`Microsatellite Instability`)[3:21],
  ...
  ## tumor_immune
  tcga_immune_id <- lapply(tcga_immune_value, function(x) {
    list(all = sort(colnames(x)[-1]))
  })
  ...
  ## tumor_pathway
  tcga_pathway_id <- lapply(tcga_pathway_value, function(x) {
    list(all = sort(colnames(x)[-1]))
  })
  ...
  pcawg_id_option$`Tumor index` <- list(
    `Tumor Purity` = list(
      all = c("purity", "ploidy", "purity_conf_mad", "wgd_status", "wgd_uncertain"),
      default = "purity"
    )
  )
  pcawg_id_option$`Phenotype data` <- list(
    `Clinical Phenotype` = list(
      all = c("Age", "Gender", "Type"),
      default = "Age"
    ),
    `Custom metadata` = list(all = NULL, default = NULL)
  )
  ...
  ccle_id_option$`Tumor index` <- list(
    `Tumor Purity` = list(
      all = c("Purity", "Ploidy", "Genome Doublings", "Lineage"),
      default = "Purity"
    )
  )
...
mol_quick_analysis <- function(molecule, data_type, out_dir = ".", out_report = FALSE) {
  ...
  cor_res <- lapply(c("Tumor index", "Immune Infiltration", "Pathway activity"), function(L1_type) {
    ...
    cor_L1 <- lapply(L2_types, function(L2_type) {
      ...
      L3_ids <- L3_ids[sapply(L3_ids, function(id) {
        class(value_df[, id])
      }) != "character"]
  ...
  sur_res <- lapply(c("OS", "DSS", "DFI", "PFI"), function(event) {
    ...
    sur_merge <- lapply(unique(data_merge$type), function(type) {
      ...
      if (length(unique(sur_dat$group)) == 1) {
        return(NA)
      }
  ...
  if (out_report) {
    ...
    file.copy(
      from = system.file("rmd", "report_template.Rmd", package = "UCSCXenaShiny"),
