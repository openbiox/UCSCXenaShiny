...
analyze_gene_drug_response_asso2 <- function(gene_list, 
  ...
  colnames(df)[1:8] <- c("gene", "cell_line", "expression", "tissue", 
  ...
  anova_fit <- function(data, manova = FALSE) {
    pval <- tryCatch({
      if (manova) {
        md <- stats::manova(cbind(response, Slope) ~ tissue + expression, data = data) %>% 
          summary()
        md$stats["expression", 6]
      } else {
        md <- stats::aov(response ~ tissue + expression, data = data) %>% 
          summary()
        md[[1]]["expression", 5]
      }
    }, error = function(e) {
    ...
    effect <- data %>% 
      ...
      dplyr::summarise(response = mean(response, na.rm = TRUE), .groups = "drop") %>%  # 先求每个组织中的均值
      tidyr::pivot_wider(id_cols = "tissue", names_from = "group", values_from = "response") %>% 
      dplyr::mutate(effect = .data$High / .data$Low) %>% # 再求高低组比值
      dplyr::pull("effect") %>% 
  ...
  out <- df %>% 
    ...
    dplyr::mutate(pval = purrr::map_dbl(.data$mod, "pval"),
...
vis_gene_drug_response_asso2 <- function(
  Gene = "TP53", response_type = c("IC50","EC50","Amax","ActArea"),
  ...
  df <- analyze_gene_drug_response_asso2(
    Gene, response_type = response_type,
    tissue = tissue, manova = manova,
    combine = TRUE) # Combine as a signature if more than 1 gene
  ...
  p <- ggplot(data = df, aes_string(
    ...
    labs(x = paste0("Gene ", response_type, " log10(effect)"), 
         y = "-log10(P-value)") +
