ui.modules_pcawg_cor_m2o = function(id) {
	ns = NS(id)
	fluidPage(
		fluidRow(
			# 初始设置
			...
			column(
				...
				wellPanel(
					...
					multi_upload_UI(ns("multi_upload2cor"),
						button_name = "Query batch data(x-axis)", id_option = pcawg_id_option),
					br(),br(),
...
server.modules_pcawg_cor_m2o = function(input, output, session) {
	ns <- session$ns
	# 记录选择癌症
